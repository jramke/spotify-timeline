...
export function useMouseStickElement<T extends HTMLElement = HTMLElement>({ label, sublabel, childRef }: UseMouseStickElementProps<T> = {}): RefObject<T> {
	...
	useEffect(() => {	
		...
		registerStickElement({ element, childElement, label, sublabel });
