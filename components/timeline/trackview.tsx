	...
	return (
		<div className="relative h-full">
			...
			<div className="absolute top-0 inline-block z-10">
				<motion.h2 layoutId="trackview-title" className="text-lg font-bold tracking-tight leading-none">
					{formatter.format(new Date(currentTrackGroup.group.date))}
				</motion.h2>
				<motion.span layoutId="trackview-subtitle" className="text-muted-foreground text-sm">
						...
						{currentTrackGroup.group!.tracks.map((track, index) => {
							...
							return (
								<div key={index}>
									...
									>
										<Image src={track.image.url} alt={`${track.name} Cover`} className="rounded" width={400} height={400} />
									</motion.div>
	...
	return (
		<div className="absolute bottom-4 left-1/2 -translate-x-1/2" style={{ perspective: 300 }}>
			<motion.div
				...
				style={{ borderRadius: 8, rotateY }}
				className="bg-card shadow-inner-shadow-float p-3 flex gap-4 items-center"
			>
				<button
					onKeyDown={(e) => (e.key === 'Enter' || e.code === 'Space') && x.set(10)}
					onKeyUp={resetRotation}
					onClick={() => setGroup(prevTrackGroup, true)}
					className={cn(textLinkClass, !prevTrackGroup && 'opacity-30 pointer-events-none')}
					disabled={!prevTrackGroup}
				>
					Previous
				</button>
				<button
					onKeyDown={(e) => (e.key === 'Enter' || e.code === 'Space') && x.set(90)}
					onKeyUp={resetRotation}
					onClick={() => setGroup(nextTrackGroup, true)}
					className={cn(textLinkClass, !nextTrackGroup && 'opacity-30 pointer-events-none')}
					disabled={!nextTrackGroup}
				>
					Next
				</button>
			</motion.div>
	...
	return (
		<div className="flex items-center gap-6">
				...
				{currentTrackGroup.group.tracks.map((track, index) => {
					...
					return (
						<motion.div
							key={index}
							layoutId={`trackview-cover-wrapper-${index}`}
							className={cn('size-full absolute grid place-items-center rounded shadow-inner-shadow-float-light')}
							style={{
						...
						>
							<Image src={track.image.url} alt={`${track.name} Cover`} className={cn('size-full rounded')} width={100} height={100} />
						</motion.div>
					);
				})}
			</div>
			<div>
				...
				<motion.div layoutId="trackview-subtitle" className="text-muted-foreground text-sm">
					{currentTrackGroup.group.value} Track
					{currentTrackGroup.group.value === 1 ? '' : 's'}
				</motion.div>
			</div>
	...
	return (
		<div className="fixed inset-4 pointer-events-none flex items-end justify-center">
			<motion.div
				...
				className={cn('inline-block pointer-events-auto shadow-inner-shadow-float bg-card p-4 mx-auto', currentTrackGroup.open ? 'size-full' : '')}
			>
				<AnimatePresence>{currentTrackGroup.open ? <TrackViewGrid /> : <TrackViewTeaser />}</AnimatePresence>
			</motion.div>
		</div>
	);
}
