			...
			<ul className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
				{playlists.map((playlist) => {
					return (
						<li key={playlist.id}>
							<Link
								href={`/timeline/${playlist.id}`}
								className="relative scale-[1.001] block spring-bounce-40 spring-duration-300 hover:scale-[1.02] focus-visible:scale-[1.02]"
							>
								{playlist?.images?.[0].url ? (
									<Image
										className="rounded shadow-inner-shadow-float aspect-square"
